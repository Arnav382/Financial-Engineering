...
def annual_ret(ret):
    ...
    return annual_ret
def daily_return(ret):
    return np.prod(ret+1)**(1/ret.shape[0])-1
...
def plot_ef2(n_points,er,cov,style="._."):
    if er.shape[1]!=2:
        raise ValueError("can only plot for 2 assets, not {}".format(er.shape[1]))                     
    weights=[np.array([w,1-w]) for w in np.linspace(0,1,n_points)]
    rets = [portfolio_return(w, annual_ret(er)) for w in weights]
...
def minimize_vol(target_return, er, cov):
    ...
    """
    n = er.shape[1] # total stocks in the portfolio
    init_guess = np.repeat(1/n, n)
    bounds = ((0.0, 1.0),)*n
    args = (cov, )
    
    # construct the constraints
    weight_sum_to_one = {'type': 'eq', 'fun': lambda weights: np.sum(weights) - 1}
    return_constraint = {'type': 'eq', 'fun': lambda weights: portfolio_return(weights,annual_ret(er))-target_return}
    constraints = (weight_sum_to_one, return_constraint)
    
    weights = minimize(portfolio_vol, init_guess, args=args, method="SLSQP", constraints=constraints, bounds=bounds)
    return weights.x.round(4)
...
def optimal_weights(n_points, er, cov):
    ...
    er:-dataframe  of daily returns
    """
    target_rs = np.linspace(annual_ret(er).min(), annual_ret(er).max(), n_points)
